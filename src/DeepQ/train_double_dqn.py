...
for episode in range(episodes):
    ...
    while True:
        ...
        if len(buffer) >= batch_size:
            ...
            with torch.no_grad():
                ...
                next_q = target_model(next_states).gather(
    ...
    if total_reward > best_reward + 1:
        ...
        no_improve = 0
        torch.save(model.state_dict(), "best_double_dqn.pth")
    else:
        no_improve += 1
...
model.load_state_dict(torch.load("best_double_dqn.pth"))
...
sumA_original = env.sumA * scale
sumB_original = env.sumB * scale
