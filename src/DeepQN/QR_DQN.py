...
items_raw = np.loadtxt(
    r"C:\Users\Thriveedh\Downloads\mdtwnpp_500_20b.txt",
    skiprows=1
)
...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size, beta=0.4):
        ...
        # ✅ Only valid entries
        valid_priorities = self.tree.tree[-self.tree.capacity:][:self.tree.n_entries]

        min_prob = valid_priorities.min() / (total + 1e-8)
    ...
    def update_priorities(self, idxs, priorities):
        for idx, priority in zip(idxs, priorities):
            priority = float(np.clip(priority, 1e-6, None))
